...
- 对内：
  - data_source in ("mock", "huxley2") → 走新链路（TrainSource → TrainToUKBinding）
  - transportapi → 显式委托旧 TrainProvider.fetch()（整体，不调用私有方法）
...
class TrainSourceBridge(BaseProvider):
    ...
    对 mock / huxley2 路径启用 Source → Binding 链路。
    """
    ...
    def __init__(self, config: dict = None):
        ...
        # transportapi 路径显式回退：整体委托旧 TrainProvider，不感知其内部实现
    ...
    def fetch(self) -> BoardContent:
        """
        mock / huxley2 路径：走新链路 TrainSource → TrainToUKBinding。
        transportapi 路径：委托旧 TrainProvider.fetch()，接触点仅为 fetch()。
        """
        data_source = self.config.get("data_source", "mock")
        if data_source in ("mock", "huxley2"):
